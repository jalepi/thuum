 ...
 *
 * @param decorator - An async wrapper function that receives the original function (with `this` already bound) and its arguments.
 * @returns A decorator that can be applied to any compatible async function, preserving its signature.
 *
 * @example
 * ```ts
 * const withLogging = decorate(async (fn, ...args) => {
